...
def motion_update(mu_bar, sigma_bar, u, dt, alphas):
    vt = u[0]
    wt = u[1]
...
def sensor_reading(mu_bar, sigma_bar, z, Q, landmarks, len_z):
    viewable_landmarks = []
    new_z = np.zeros(len_z, 3)
...
def EKF(robot_odometry, initial_x, initial_y, initial_theta, robot_measurements):
    ...
    for i in tqdm(range(1, robot_odometry.shape[0])):
        ...
        mu_bar, sigma_bar = motion_update(mu_bar, sigma_bar, [robot_odometry[i][1], robot_odometry[i][2]], dt, alphas)
        
        # Check to see if we have a sensor reading at this time step
        if robot_odometry[i][0] + 0.2 > robot_measurements[measurement_idx][0]:
            len_z = 1
            while (robot_measurements[measurement_idx][0] == robot_measurements[measurement_idx+len_z][0]):
                len_z += 1
        
            # Correction Step
            measurement_flag = True
        
            mu_bar, sigma_bar = sensor_reading(mu_bar, sigma_bar, robot_measurements[measurement_idx][1:], Q_t, landmark_dict, len_z)
